...
class TestDatabase(unittest.TestCase):
    ...
    def test_create_database(self):
        ...
        self.assertTrue(os.path.isfile(
            self.database.folder + "/" + self.database.fileName))
    ...
    def test_add_field(self):
        self.database.addField(
            self.fields[0], self.ttest[0], self.fieldTypes[0])
    ...
    def test_get_most_probable_group_from_entry(self):
        ...
        self.__check_group_distribution__(newEntry, 0.3)
    ...
    def test_add_entry_to_biased_database(self):
        ...
        self.__check_group_distribution__(newEntry, 0.0)

    def test_add_entry_to_empty_database(self):
        self.database.addFields(self.fields, self.ttest, self.fieldTypes)
        self.database.groups = self.groups.copy()
        newEntry = {'SubjectId': 's01', 'Age': '65'}
        self.__check_group_distribution__(newEntry, 0.5)

    def test_add_entry_to_one_line_database(self):
        self.database.addFields(self.fields, self.ttest, self.fieldTypes)
        self.database.groups = self.groups.copy()
        self.database.addEntryWithGroup(self.possibleEntries[0])
        self.__check_group_distribution__(self.possibleEntries[1], 0.5)
    ...
    def test_add_entry_to_two_lines_database(self):
        ...
        self.database.addEntryWithGroup(self.possibleEntries[0])
        self.database.addEntryWithGroup(self.possibleEntries[1])
        self.__check_group_distribution__(self.possibleEntries[3], 0.5)
    ...
    def test_add_entry_to_three_lines_database(self):
        ...
        self.database.addEntryWithGroup(self.possibleEntries[1])
        self.database.addEntryWithGroup(self.possibleEntries[2])
        self.__check_group_distribution__(self.possibleEntries[3], 0.81)
    ...
    def __check_correct_DB_Info__(self):
        ...
        for field in self.fields:
            ...
            self.assertEqual(self.database.getFieldTypeFromField(
                field), self.fieldTypes[fieldIndex])
            self.assertEqual(self.database.getTtestFromField(
                field), self.ttest[fieldIndex])
            fieldIndex += 1

    def __check_group_distribution__(self, newEntry, expectedFirstGroupProbability):
        groups = []
        countGroup = dict()
        ...
        proba = countGroup[self.groups[0]] / \
            (countGroup[self.groups[1]] + countGroup[self.groups[1]])
    ...
    def test_add_rejected_entry(self):
        ...
        self.assertEqual(
            self.database.entries[self.database.rejectedEntries[0]], self.possibleEntries[2])
    ...
    def test_remove_rejected_entry(self):
        ...
        self.assertEqual(len(self.database.rejectedEntries), 0)
    ...
    def test_add_subject_where_max_difference_is_reached(self):
        ...
        expectedPValues = self.database.getGroupsProbabilitiesFromNewEntry(
            entry)
